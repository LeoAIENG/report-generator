"""
Data preprocessing module for Report 3.

This module handles the preprocessing of loan data from JSON format,
including data flattening, field mapping, and officer name normalization.
"""
...
def loan_df_from_records(loan_json_path):
    """
    Converts a JSON file containing loan data into a pandas DataFrame.

    Args:
            loan_json_path (str): The path to the JSON file containing loan data.
...
def preprocess(loan_json_path, credit_excel_path):
    """
    Preprocesses the loan data and credit data to create a merged DataFrame.

    Args:
            loan_json_path (str): The path to the JSON file containing loan data.
            credit_excel_path (str): The path to the Excel file containing credit data.

    Returns:
            pandas.DataFrame: A DataFrame containing the merged loan and credit data.
    """
    ...
    credit_long = credit_df.melt(
        id_vars="Loan Officer", var_name="Scenario", value_name="Credit Pulls"
    ).dropna()
    ...
    closed_officers = [
        first_last(loan["fields"]["317"]) for loan in closed_loans if loan["fields"].get("317")
    ]
    ...
    credit_totals_grouped = (
        credit_totals.groupby("Normalized Name")["Credit Pulls"].sum().reset_index()
    )
    closed_grouped = (
        closed_per_officer.groupby("Normalized Name")["Closed Loans"].sum().reset_index()
    )

    merged = pd.merge(
        credit_totals_grouped, closed_grouped, on="Normalized Name", how="outer"
    ).fillna(0)
    merged["Credit Pulls"] = merged["Credit Pulls"].astype(int)
    merged["Closed Loans"] = merged["Closed Loans"].astype(int)
    merged["Close Rate (%)"] = merged.apply(
        lambda r: round((r["Closed Loans"] / r["Credit Pulls"]) * 100, 1)
        if r["Credit Pulls"] > 0
        else None,
        axis=1,
    )
    merged["Loans per Pull"] = merged.apply(
        lambda r: round((r["Closed Loans"] / r["Credit Pulls"]), 2)
        if r["Credit Pulls"] > 0
        else None,
        axis=1,
    )
    merged["Loan Officer"] = merged["Normalized Name"].apply(
    ...
    officer_branch_amount_df = pd.DataFrame(
        officer_branch_amount_triplets, columns=["Normalized Name", "ORGID", "Loan Amount"]
    )

    officer_branch_map = (
        officer_branch_amount_df.groupby("Normalized Name")["ORGID"]
        .agg(lambda x: Counter(x).most_common(1)[0][0])
        .reset_index()
    )
    officer_amount_map = (
        officer_branch_amount_df.groupby("Normalized Name")["Loan Amount"].sum().reset_index()
    )

"""
Data visualization module for Report 3.

This module contains functions for creating various charts and visualizations
used in Report 3, including loan officer efficiency plots, closed loan volume
plots, and branch-based closed loan volume comparisons.
"""
...
def generate_shades(base_hex, n_shades):
    """
    Generates a list of shades of a given color.
...
def plot_loan_officer_by_efficiency(
    table,
    image_path,
    prefix,
    time_label,
    show_plots=False,
    graph_name="loan_officer_by_efficiency",
):
    """
    Plots a table of loan officer efficiency.

    Args:
            table (pd.DataFrame): The table to plot.
            image_path (Path): The path to save the image.
            prefix (str): The prefix of the image.
            time_label (str): The time label of the image.
    ...
    fig.text(
        0.5,
        0.975,
        f"{time_label} Loan Officers by Efficiency – Top 30",
        ha="center",
        fontsize=13,
        weight="bold",
    )
    fig.text(
        0.5,
        0.02,
        "Efficiency is measured by Loans per Pull (Closed Loans / Credit Pulls)",
        ha="center",
        fontsize=12,
        style="italic",
    )
...
def plot_closed_pulls(
    table, image_path, prefix, time_label, show_plots=False, graph_name="closed_pulls_top30"
):
    """
    Plots a bar chart of closed loans vs. credit pulls for the top 30 loan officers.
    ...
    for i in x:
        ...
        ax.bar(
            i + bar_width, top_30["Credit Pulls"].iloc[i], width=bar_width, color=pulls_shades[i]
        )
        ax.text(
            i,
            top_30["Closed Loans"].iloc[i] + 1,
            f"{top_30['Closed Loans'].iloc[i]}",
            ha="center",
            fontsize=9,
        )
        ...
        ax.text(
            i + bar_width,
            top_30["Credit Pulls"].iloc[i] + 1,
            f"{top_30['Credit Pulls'].iloc[i]}",
            ha="center",
            fontsize=9,
        )
    ...
    ax.set_title(
        f"{time_label} Closed Loans vs. Credit Pulls – Top 30 Loan Officers",
        fontsize=13,
        weight="bold",
    )
...
def plot_closed_pulls_by_branch(
    table, image_path, prefix, time_label, show_plots=False, graph_name="closed_pulls_by_branch"
):
    """
    Plots a bar chart of closed loans vs. credit pulls for each branch.
    ...
    for i in x:
        ...
        ax.bar(
            i + bar_width,
            sorted_branch["Credit Pulls"].iloc[i],
            width=bar_width,
            color=pulls_shades[i],
        )

        # Add value labels
        ax.text(
            i,
            sorted_branch["Closed Loans"].iloc[i] + 1,
            f"{sorted_branch['Closed Loans'].iloc[i]}",
            ha="center",
            fontsize=9,
        )
        ax.text(
            i + bar_width,
            sorted_branch["Credit Pulls"].iloc[i] + 1,
            f"{sorted_branch['Credit Pulls'].iloc[i]}",
            ha="center",
            fontsize=9,
        )
    ...
    ax.set_title(
        f"{time_label} Closed Loans vs. Credit Pulls by Branch", fontsize=14, weight="bold"
    )

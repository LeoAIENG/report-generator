...
class DataRetriever:
    ...
    def is_funded_last_month(
        self, funding_date_str: str, start_date: datetime, end_date: datetime
    ) -> bool:
    ...
    def retrieve_loan_field_data(
        self, folders: List[str], output_path: str
    ) -> List[Dict[str, Any]]:
            ...
            for loan_id in tqdm(loan_ids, desc=f"Fetching fields from '{folder}'"):
                ...
                if response.status_code == 200:
                    ...
                    loan_field_data.append(
                        {"loanId": loan_id, "folder": folder, "fields": loan_data}
                    )
                else:
    ...
    def extract_month_year(self, date_str: str) -> Optional[str]:
        try:
            ...
            print(f"⚠️ Error extracting month year: {date_str} - {e}")
            return None

    def analyze_date_fields(
        self, loan_field_data: List[Dict[str, Any]]

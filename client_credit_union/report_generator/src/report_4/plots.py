...
def plot_closed_loan_volume(df, title_prefix, image_path, prefix, show_plots=False):
  grouped = df.groupby("underwriter").agg(
...
def plot_product_type_distribution(df, title_prefix, image_path, prefix, show_plots=False):
  product_distribution = (
      df.groupby(["underwriter", "product_category"])
      .size()
  ...
  ax.set_title(f"{title_prefix} Closed Loan Product Type Distribution by Underwriter", fontsize=13, weight='bold')
...
def generate_product_type_summary_table(df, title_prefix, image_path, prefix, show_plots=False):
  ...
  ax.text(0.5, 0.80, f"{title_prefix} Closed Loan Product Type Distribution by Underwriter",
...
def plot_avg_days_to_close(df, title_prefix, image_path, prefix, show_plots=False):
  ...
  ax = sns.barplot(data=avg_days, x="underwriter", y="avg_days_to_close", palette=colors, legend=False, hue="underwriter")

  plt.title(f"{title_prefix} Average Days from Submission to Clear to Close per Underwriter", fontsize=13, weight="bold")
...
def plot_loans_missing_submittal(df, title_prefix, image_path, prefix, show_plots=False):
  ...
  ax = sns.barplot(x=count_by_underwriter.index, y=count_by_underwriter.values, palette=colors, hue=count_by_underwriter.index, legend=False)
